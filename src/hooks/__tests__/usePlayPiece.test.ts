...
import { Board, PlayerTurn } from "types";
...
const render = () => {
  ...
  return {
    ...
    assertGame: (player: PlayerTurn, gameOver: boolean, board: Board) => {
...
test("should win with 4 in a row diagonally", () => {
  ...
  // plays
  [3,2,2,1,0,1,1,5,0,0,0].forEach(play);

  // Player 1 won the game!
  assertGame(1, true, [[1, 1, 2, 1], [2, 2, 1], [2, 1], [1], [], [2], []]);
});
